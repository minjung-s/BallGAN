...
# BG setting
@click.option('--bg_ver',       help='version of BG network',                                   type=click.Choice(['ver0', 'ver1', 'ver2']), default='ver1', show_default=True)
...
@click.option('--sigma_act',    help='activation function of sigma branch', metavar='STR',      type=click.Choice(['linear', 'relu', 'lrelu']), required=False, default='linear')
...
@click.option('--fg_density_spatial_reg',    help='Foreground density regularization strength. (from BallGAN)', metavar='FLOAT', type=click.FloatRange(min=0), default=0, required=False, show_default=True)
@click.option('--fg_density_reg',    help='Foreground density regularization strength. (from BallGAN)', metavar='FLOAT', type=click.FloatRange(min=0), default=0, required=False, show_default=True)
@click.option('--fg_density_reg_scheduler',    help='Foreground density regularization scheduling', metavar='BOOL',  type=bool, required=False, default=False)
@click.option('--bg_density_reg_scheduler',    help='Bakcground transmittance regularization scheduling', metavar='BOOL',  type=bool, required=False, default=False)
@click.option('--bg_transmittance_reg',    help='Bakcground transmittance regularization strength. (from BallGAN)', metavar='FLOAT', type=click.FloatRange(min=0), default=0, required=False, show_default=True)
@click.option('--bg_transmittance_reg_start',    help='Bakcground transmittance regularization strat img. (from BallGAN)', metavar='INT', type=click.IntRange(min=0), default=0, required=False, show_default=True)
@click.option('--density_reg',    help='Density regularization strength.', metavar='FLOAT', type=click.FloatRange(min=0), default=0., required=False, show_default=True)
...
def main(**kwargs):
    ...
    c.G_kwargs = dnnlib.EasyDict(class_name=None, z_dim=512, w_dim=512, z_bg_dim=opts.z_bg_dim, w_bg_dim=opts.w_bg_dim, mapping_kwargs=dnnlib.EasyDict()) #
    ...
    rendering_options = {
        ...
        'sigma_act': opts.sigma_act, # strength of density regularization
        'fg_density_spatial_reg' : opts.fg_density_spatial_reg, # strength of foreground ray entropy regularization from BallGAN
        'fg_density_reg' : opts.fg_density_reg, # strength of foreground density regularization from BallGAN
        'fg_density_reg_scheduler' : opts.fg_density_reg_scheduler,
        'bg_density_reg_scheduler' : opts.bg_density_reg_scheduler,
        'bg_transmittance_reg' : opts.bg_transmittance_reg, # strength of background transmittance regularization from BallGAN
        'bg_transmittance_reg_start' : opts.bg_transmittance_reg_start, # start img of background transmittance regularization from BallGAN
    ...
    if opts.bg_ver == "ver0":
        ...
        c.G_kwargs.z_bg_dim = BG_options['z_dim']
        c.G_kwargs.w_bg_dim = BG_options['w_dim']
        ...
        c.G_kwargs.z_bg_dim = BG_options['mapping_kwargs']['z_dim']
        c.G_kwargs.w_bg_dim = BG_options['mapping_kwargs']['w_dim']
    ...
    c.G_kwargs.BG_kwargs = BG_options
    ...
    if opts.resume is not None:
        ...
        c.resume_kimg = int(opts.resume.split('-')[-1][:-4]) * 1000
