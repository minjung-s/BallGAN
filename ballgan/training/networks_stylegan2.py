...
class MappingNetwork(torch.nn.Module):
    def __init__(self,
        ...
        num_ws          = 5,        # Number of intermediate latents to output, None = do not broadcast.
...
@persistence.persistent_class
class ToRGBLayer(torch.nn.Module):
    def __init__(self, in_channels, out_channels, w_dim=0, kernel_size=1, conv_clamp=None, channels_last=False, mode='2d', initial='rand', **unused):
        super().__init__()
        self.conv_clamp = conv_clamp
        self.mode = mode
    ...
    def forward(self, x, w=None, fused_modconv=True):
        if w is not None:
            ...
            if x.ndim == 2:
                ...
                x = F.conv2d(x, self.weight[:,:,None,None].to(x.dtype), self.bias.to(x.dtype))
        return x
