...
class StyleGAN2Loss(Loss):
    ...
    def run_G(self, z, z_bg, c, swapping_prob, neural_rendering_resolution, update_emas=False):
        ...
        if self.G.BG_kwargs['bg_ver'] == "ver0" :
            gen_output = self.G.synthesis(ws, c, z_bg=z_bg, ws_bg=None, neural_rendering_resolution=neural_rendering_resolution, update_emas=update_emas)
        elif self.G.BG_kwargs['bg_ver'] == "ver1" :
            ws_bg = self.G.mapping_bg(z_bg , update_emas=update_emas)
            gen_output = self.G.synthesis(ws, c, z_bg=None, ws_bg=ws_bg, neural_rendering_resolution=neural_rendering_resolution, update_emas=update_emas)
    ...
    def exp_wegiht_scheduler(self, cur_nimg):
        return math.exp(cur_nimg/5000)/20

    def g(self, x):
            return 1./x

    def t2s(self, pts, tn, tf):
        return (self.g(pts)-self.g(tn))/(self.g(tf)-self.g(tn))

    def get_fg_reg(self, output):


        di = output['fg_di'].squeeze(-1).to(torch.float16)
        fg_density = torch.sigmoid(output['fg_density'].reshape(di.shape)).to(torch.float16)

        fg_s_val = self.t2s(di,torch.min(di, dim=2, keepdim=True)[0].repeat(1,1,di.shape[-1]),torch.max(di, dim=2, keepdim=True)[0].repeat(1,1,di.shape[-1]))


        weight_table = torch.matmul(fg_density.unsqueeze(-1),fg_density.unsqueeze(-2))
        s_dist_all = torch.cdist(fg_s_val.unsqueeze(-1),fg_s_val.unsqueeze(-1),p=2)

        tmp = torch.mul(weight_table,s_dist_all)
        fg_reg_loss = torch.mean(torch.sum(torch.triu(tmp,diagonal=1),(-1,-2)),1)

        di = output['fg_di'].squeeze(-1)
        fg_density = torch.sigmoid(output['fg_density'].reshape(di.shape))

        s_dist_one = di[...,1:]-di[...,:-1]
        s_dist_one = torch.cat([s_dist_one,s_dist_one[...,-1].unsqueeze(-1)],dim=-1)
        reg_one = torch.sum(torch.mul(fg_density,s_dist_one),-1)/3

        fg_reg_loss +=  torch.mean(reg_one,1)

        return fg_reg_loss

    def get_fg_spatial_reg(self, output):
        ent = -torch.sum(F.softmax(output['fg_density'], dim=-1)*F.log_softmax((output['fg_density']+1e-30),dim=-1),-1).mean(-1).mean(-1).unsqueeze(-1)
        return ent

    def get_bg_transmittance_reg(self, output):
        bg_transmittance = output['bg_transmittance'].squeeze(-1)
        B, S, _ = bg_transmittance.shape
        binarization = (torch.min(bg_transmittance,1-bg_transmittance).sum(1)/S)

        return binarization

    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_z_bg, gen_c, gain, cur_nimg, cur_tick):
        assert phase in ['Gmain', 'Greg', 'Greg_fgdensity', 'Greg_fgdensity_spatial', 'Greg_bgtransmittance', 'Gboth', 'Dmain', 'Dreg', 'Dboth']
        ...
        # FG density Regularization from BallGAN
        if phase in ['Greg_fgdensity', 'Gboth'] and self.G.rendering_kwargs.get('fg_density_reg', 0) > 0:
            gen_img, _ = self.run_G(gen_z, gen_z_bg, gen_c, swapping_prob=swapping_prob, neural_rendering_resolution=neural_rendering_resolution)
            if self.G.rendering_kwargs['fg_density_reg_scheduler'] and cur_nimg < 15000:
                loss_G_fgdensity = self.get_fg_reg(gen_img)*self.exp_wegiht_scheduler(cur_nimg)*self.G.rendering_kwargs['fg_density_reg']
            else :
                loss_G_fgdensity = self.get_fg_reg(gen_img)*self.G.rendering_kwargs['fg_density_reg']
            loss_G_fgdensity.mean().mul(gain).backward()
            training_stats.report('Loss/reg_fgdensity', loss_G_fgdensity.mean())

        # FG spatial entropy Regularization from BallGAN
        if phase in ['Greg_fgdensity_spatial', 'Gboth'] and self.G.rendering_kwargs.get('fg_density_spatial_reg', 0) > 0:
            gen_img, _ = self.run_G(gen_z, gen_z_bg, gen_c, swapping_prob=swapping_prob, neural_rendering_resolution=neural_rendering_resolution)
            loss_G_fgdensity = self.get_fg_spatial_reg(gen_img)*self.G.rendering_kwargs['fg_density_spatial_reg']
            loss_G_fgdensity.mean().mul(gain).backward()
            training_stats.report('Loss/reg_fg_onray_density', loss_G_fgdensity.mean())

        # BG transmittance Regularization from BallGAN
        if phase in ['Greg_bgtransmittance', 'Gboth'] and self.G.rendering_kwargs.get('bg_transmittance_reg', 0) > 0 and cur_nimg>=self.G.rendering_kwargs['bg_transmittance_reg_start']:
            gen_img, _ = self.run_G(gen_z, gen_z_bg, gen_c, swapping_prob=swapping_prob, neural_rendering_resolution=neural_rendering_resolution)
            if self.G.rendering_kwargs['bg_density_reg_scheduler'] and cur_nimg < 15000:
                loss_G_bgtransmittance = self.get_bg_transmittance_reg(gen_img)*self.exp_wegiht_scheduler(cur_nimg)*self.G.rendering_kwargs['bg_transmittance_reg']
            else:
                loss_G_bgtransmittance = self.get_bg_transmittance_reg(gen_img)*self.G.rendering_kwargs['bg_transmittance_reg']
            loss_G_bgtransmittance.mean().mul(gain).backward()
            training_stats.report('Loss/reg_bgtransmittance', loss_G_bgtransmittance.mean())

        # Density Regularization
        if phase in ['Greg', 'Gboth'] and self.G.rendering_kwargs.get('density_reg', 0) > 0 and self.G.rendering_kwargs['reg_type'] == 'l1': #EG3D default
            if swapping_prob is not None:
