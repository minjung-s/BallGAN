...
class SynthesisBlockNoUp(torch.nn.Module):
    ...
    def forward(self, x, img, ws, force_fp32=False, fused_modconv=None, update_emas=False, **layer_kwargs):
        ...
        if self.in_channels == 0:
            ...
            # print("Before modulated_conv 0 : {}".format(x.shape))
            x = self.conv0(x, next(w_iter), fused_modconv=fused_modconv, **layer_kwargs)
            # print("After modulated_conv 0 : {}".format(x.shape))
            x = self.conv1(x, next(w_iter), fused_modconv=fused_modconv, **layer_kwargs)
            # print("After modulated_conv 1 : {}".format(x.shape))
        ...
        if self.is_last or self.architecture == 'skip':
            # print("Before torgb : {}".format(x.shape))
            y = self.torgb(x, next(w_iter), fused_modconv=fused_modconv)
            y = y.to(dtype=torch.float32, memory_format=torch.contiguous_format)
            img = img.add_(y) if img is not None else y
            # print("After torgb : {}".format(img.shape))
...
#             rgb = torch.nn.functional.interpolate(rgb, size=(self.input_resolution, self.input_resolution),
#                                                   mode='bilinear', align_corners=False, antialias=self.sr_antialias)
#         print("Before block0 : {} | {}".format(x.shape, rgb.shape))
#         x, rgb = self.block0(x, rgb, ws, **block_kwargs)
#         print("After block0 : {} | {}".format(x.shape, rgb.shape))
#         x, rgb = self.block1(x, rgb, ws, **block_kwargs)
#         print("After block1 : {} | {}".format(x.shape, rgb.shape))
#         return rgb

#----------------------------------------------------------------------------
@persistence.persistent_class
class SynthesisBlock_WOmoudlation(torch.nn.Module):
    def __init__(self,
        in_channels,                            # Number of input channels, 0 = first block.
        out_channels,                           # Number of output channels.
        w_dim,                                  # Intermediate latent (W) dimensionality.
        ...
        **layer_kwargs,                         # Arguments for SynthesisLayer.
        ...
        if in_channels != 0:
            self.conv0 = Conv2dLayer(in_channels, out_channels, resolution=resolution, up=2,
